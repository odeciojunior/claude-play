# Plugin-Reviewer Subagent Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create a plugin-reviewer subagent that reviews plugin submissions with 3-phase checks: structural validation (reusing validate-plugin logic), content quality, and security/policy review.

**Architecture:** A single Markdown agent file at `.claude/agents/plugin-reviewer.md`. When dispatched as a subagent, it reads the plugin files, runs the validate-plugin checks inline (same logic, not invoking the skill), then performs content quality and security reviews. Outputs a structured review report.

**Tech Stack:** Claude Code agent file (Markdown), Grep/Glob/Read tools for analysis

---

### Task 1: Create the plugin-reviewer agent file

**Files:**
- Create: `.claude/agents/plugin-reviewer.md`
...
Run: `wc -l .claude/agents/plugin-reviewer.md`
Expected: ~120 lines

**Step 3: Commit**

```bash
git add .claude/agents/plugin-reviewer.md
git commit -m "feat: add plugin-reviewer subagent"
```
...
**Step 1: Add agents section after the Hooks section**

In `CLAUDE.md`, after the `## Hooks` section and before `## Contributing`, add:
...
```bash
git add CLAUDE.md
git commit -m "docs: add agents section to CLAUDE.md"
```

---

### Task 3: Test the agent against an existing plugin

**Step 1: Dispatch the agent to review `system-health-check`**

Use the Agent tool to dispatch the plugin-reviewer subagent with this prompt:
"Review the plugin `system-health-check` following the instructions in `.claude/agents/plugin-reviewer.md`."

**Step 2: Verify output format**

Check that the output contains all sections:
- `## Plugin Review: system-health-check`
- `### Structural Checks` with 7 checks
- `### Content Quality` with 3 items
- `### Security & Policy` with 5 items
- `### Verdict:` with APPROVE or REQUEST CHANGES

**Step 3: Dispatch again for `marketplace-tools`**

Review the second plugin to verify it works across different plugin structures (marketplace-tools has 2 skills vs system-health-check's 1).

Verify the output covers both skills in the Content Quality section.
